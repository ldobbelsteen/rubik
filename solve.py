    ...
    for s in range(k):
        ...
        for i, (x_hi, y_hi, z_hi, r, cw) in enumerate(corners[s]):
            if not move_stacking or move_stacking_single:
                ...
                solver.add(
                    next_x_hi
                    == move_mappers.z3_corner_x_hi(x_hi, y_hi, z_hi, ax, hi, dr)
                )
                solver.add(
                    next_y_hi
                    == move_mappers.z3_corner_y_hi(x_hi, y_hi, z_hi, ax, hi, dr)
                )
                solver.add(
                    next_z_hi
                    == move_mappers.z3_corner_z_hi(x_hi, y_hi, z_hi, ax, hi, dr)
                )
                solver.add(
                    next_r == move_mappers.z3_corner_r(x_hi, z_hi, r, cw, ax, hi, dr)
                )
                solver.add(
                    next_cw
                    == move_mappers.z3_corner_cw(x_hi, y_hi, z_hi, cw, ax, hi, dr)
                )
            elif s % 2 == 0:
        ...
        for i, (a, x_hi, y_hi, r) in enumerate(edges[s]):
            if not move_stacking or move_stacking_single:
                ...
                solver.add(next_a == move_mappers.z3_edge_a(a, x_hi, y_hi, ax, hi, dr))
                solver.add(
                    next_x_hi == move_mappers.z3_edge_x_hi(a, x_hi, y_hi, ax, hi, dr)
                )
                solver.add(
                    next_y_hi == move_mappers.z3_edge_y_hi(a, x_hi, y_hi, ax, hi, dr)
                )
                solver.add(next_r == move_mappers.z3_edge_r(a, next_a, r))
            elif s % 2 == 0:
