...
class VarConditions:
    def __init__(self, name: str, domain: list[int], s: int, d: int):
        ...
        # TODO: add smaller than, less than operators
        self.const_equalities = [z3.Bool(f"{name}(s + {s}) == {v}") for v in domain]
        self.const_inequalities = [z3.Bool(f"{name}(s + {s}) != {v}") for v in domain]
        self.symb_equalities = [
            z3.Bool(f"{name}(s + {s}) == {name}(s + {f})") for f in range(s + 1, d)
        ]
        self.symb_inequalities = [
            z3.Bool(f"{name}(s + {s}) != {name}(s + {f})") for f in range(s + 1, d)
        ]
    ...
    def facilitates(
        ...
        for s in range(len(self.symb_inequalities)):
            conflicting.append(
                z3.And(
                    self.symb_inequalities[s],
                    z3.Or(
                        v == next_values[s],
                        next_conditions[s].facilitates(
                            v,
    ...
    def extract_from_model(self, model: z3.ModelRef) -> list[z3.BoolRef]:
        result = []
        for v in self.const_equalities:
            if z3.is_true(model.get_interp(v)):
                result.append(v)
        for v in self.const_inequalities:
            if z3.is_true(model.get_interp(v)):
                result.append(v)
        for v in self.symb_equalities:
            if z3.is_true(model.get_interp(v)):
                result.append(v)
        for v in self.symb_inequalities:
            if z3.is_true(model.get_interp(v)):
                result.append(v)
        return result
...
def find(n: int, d: int):
    ...
    condition_count = z3.Sum(
        [
            ...
            for v in c.const_equalities
            + c.const_inequalities
            + c.symb_equalities
            + c.symb_inequalities
        ]
    )
