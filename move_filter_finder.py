...
class Variable:
    ...
    def __eq__(self, other: "Variable"):
        ...
        return self.name == other.name and self.s == other.s
...
class Condition:
    ...
    def __eq__(self, other: "Condition"):
        """Returns whether this condition is equal to another condition."""
        return (
...
def find_move_filter(n: int, k: int):
    ...
    sequences. This is not an efficient process and grows exponentially in the sequence
    length.
    """
    ...
    m = solver.model()
    filter = tuple([str(cond) for cond in filter.conditions_from_model(m)])
    print_stamped(f"found filter for {m.evaluate(filtered_count)}: {filter}")
...
if __name__ == "__main__":
    ...
    find_move_filter(args.n, args.k)
